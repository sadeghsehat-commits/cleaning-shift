...
export default function ShiftDetailPage() {
  ...
  const handleRequestTimeChange = async () => {
    if (!timeChangeForm.newStartTime || !timeChangeForm.reason) {
    ...
    try {
      const scheduledDate = new Date(shift!.scheduledDate);
      const [hours, minutes] = timeChangeForm.newStartTime.split(':');
      ...
      const response = await fetch(apiUrl(`/api/shifts/${params.id}/time-change`), {
        ...
        body: JSON.stringify({
          ...
          newEndTime: newEndDateTime ? newEndDateTime.toISOString() : undefined,
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        toast.error(data.error || 'Failed to request time change');
      }
    } catch (error) {
      toast.error('An error occurred');
    }
  ...
  const handleReportProblem = async () => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/shifts/${params.id}/problems`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleAddInstructionPhoto = async () => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/shifts/${params.id}/instruction-photos`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/shifts/${params.id}/comments`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleConfirmShift = async () => {
    try {
      const response = await fetch(apiUrl(`/api/shifts/${params.id}/confirm`), {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const canApproveTimeChange = user?.role === 'owner';
  const canRequestTimeChange = user?.role === 'admin';
  ...
  return (
    <div className="space-y-6">
                  ...
                  {user?.role === 'admin' && (
                    <button
                      onClick={async () => {
                        ...
                        try {
                          const response = await fetch(apiUrl(`/api/shifts/${params.id}/comments/${commentId}`), {
                            method: 'DELETE',
                            credentials: 'include',
                          });
                          if (response.ok) {
      ...
      {/* Time Change Request Section - Simplified for brevity */}
      <div>
        <div className="flex justify-between items-center mb-4">
            ...
            {isOperator && 
             ...
             !shift.timeChangeRequest.operatorConfirmed && (
              <div className="mt-4 flex gap-2">
                <button
                  onClick={async () => {
                    const response = await fetch(apiUrl(`/api/shifts/${params.id}/time-change/confirm`), {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      credentials: 'include',
