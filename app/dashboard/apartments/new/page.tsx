...
export default function NewApartmentPage() {
  ...
  useEffect(() => {
    ...
    // We need to check if it's available in the client
  ...
  const loadGoogleMaps = () => {
    ...
    script.onerror = () => {
      const errorMsg = 'Failed to load Google Maps. Please check your API key and ensure Places API is enabled.';
  ...
  const initializeCityAutocomplete = () => {
    ...
    if (!window.google?.maps?.places) {
      console.warn('Google Maps Places API not available. Manual city entry will be used.');
      return;
    }
  ...
  const checkAuth = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setUser(data.user);
        // Operators and viewers cannot create apartments
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-2 flex-wrap">
          ...
          <span className="text-gray-400 flex items-center">/</span>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center">Create New Apartment</h1>
        </div>
      </div>
